...
function fig = init(nii, fig, area, setunit, setviewpoint, setscanid, buttondown, ...
  ...
  if xdim > 1
    nii_view.slices.sag = origin(1);
  end
  if ydim > 1
    nii_view.slices.cor = origin(2);
  end
  if zdim > 1
    nii_view.slices.axi = origin(3);
  end
...
function nii_view = check_slices(nii_view)
  ...
  if nii_view.slices.sag > SagSize
    nii_view.slices.sag = SagSize;
  end
  if nii_view.slices.sag < 1
    nii_view.slices.sag = 1;
  end
  if nii_view.slices.cor > CorSize
    nii_view.slices.cor = CorSize;
  end
  if nii_view.slices.cor < 1
    nii_view.slices.cor = 1;
  end
  if nii_view.slices.axi > AxiSize
    nii_view.slices.axi = AxiSize;
  end
  if nii_view.slices.axi < 1
    nii_view.slices.axi = 1;
  end
  if nii_view.scanid > TimeSize
    nii_view.scanid = TimeSize;
  end
  if nii_view.scanid < 1
    nii_view.scanid = 1;
  end
