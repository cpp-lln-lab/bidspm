function BIDS_FFX_MVPA(action, degreeOfSmoothing, opt)
    ...
    case 1 % fMRI design and estimate
        ...
        for iGroup= 1:length(group)              % For each group
            ...
            for iSub = 1:group(iGroup).numSub    % For each Subject in the group
                ...
                ffxDir = getFFX_MVPA_dir(subNumber, degreeOfSmoothing, opt, 1);
                ...
                save(fullfile(JOBS_dir, ['jobs_matlabbatch_SPM12_ffx_MVPA_',...
    ...
    case 2  % CONTRASTS
        ...
        for iGroup= 1:length(group)                     % For each group
            ...
            for iSub = 1:group(iGroup).numSub           % For each subject in the group
                ...
                % ffx folder
                ffxDir = getFFXdir(subNumber, degreeOfSmoothing, opt, 1);
                
                JOBS_dir = fullfile(opt.JOBS_dir, subNumber);
                
                % Create Contrasts
                [~, contrasts] = pmConMVPA(ffxDir, opt.taskName, opt);
                ...
                save(fullfile(JOBS_dir, ...
                    ['jobs_matlabbatch_SPM12_ffx_MVPA_',...
